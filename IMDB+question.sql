...
-- (Note: The same movie can belong to multiple genres.)genregenre
...
FROM movie AS m INNER JOIN ratings AS r on
m.id=r.movie_id
LIMIT 10;
...
SELECT genre,
        ...
        AVG(AVG(duration)) over(order by genre rows 11 preceding) AS moving_avg_duration
FROM movie AS m INNER JOIN genre AS g on
m.id= g.movie_id
group by genre
order by genre;
